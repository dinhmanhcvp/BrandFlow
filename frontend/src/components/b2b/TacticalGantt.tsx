  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bento-card border border-slate-200 bg-white shadow-sm overflow-hidden p-6">
         <h3 className="font-bold text-slate-900 text-lg mb-4">{language === 'vi' ? 'B3. Ma trận Hành động' : 'B3. Action Matrix'}</h3>
         <div className="overflow-x-auto">
            <table className="w-full text-sm text-left border border-slate-200 rounded-lg overflow-hidden">
               <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                     <th className="py-3 px-4 font-bold text-slate-700">{language === 'vi' ? 'Mục tiêu' : 'Objective'}</th>
                     <th className="py-3 px-4 font-bold text-slate-700">{language === 'vi' ? 'Chiến thuật / Hành động Chính' : 'Key Tactics / Actions'}</th>
                     <th className="py-3 px-4 font-bold text-slate-700 w-32 border-l border-slate-200">{language === 'vi' ? 'Phụ trách' : 'Owner'}</th>
                     <th className="py-3 px-4 font-bold text-slate-700 w-32 text-right border-l border-slate-200">{language === 'vi' ? 'Ngân sách' : 'Budget'}</th>
                  </tr>
               </thead>
               <tbody>
                  {TASKS.map(t => (
                     <tr key={t.id} className="border-b border-slate-200 hover:bg-slate-50 transition-colors">
                        ...
                        <td className="py-3 px-4 text-slate-500 font-medium truncate max-w-xs cursor-pointer hover:text-cyan-600 transition-colors">{language === 'vi' ? 'Nhấp để mở xem chi tiết chiến thuật này...' : 'Click to view tactic details...'}</td>
         ...
         <div className="overflow-x-auto">
            <div className="min-w-[800px]">
               ...
               <div className="flex border-b border-slate-200 pb-2">
                  <div className="w-48 shrink-0 font-bold text-slate-600 text-xs uppercase tracking-wider pl-2">{language === 'vi' ? 'Giai đoạn / Nhiệm vụ' : 'Phase / Task'}</div>
