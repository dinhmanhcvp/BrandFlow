...
export default function ScreenSimulation({ iteration, feedback, onComplete }) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col md:flex-row items-center justify-center p-6 gap-6 font-sans">
      
      {/* Left: Animation Triangle */}
      <div className="w-full md:w-1/3 flex flex-col items-center justify-center">
        <div className="text-center mb-8">
          <h3 className="text-white font-bold text-xl mb-2">Neural Synergy Chamber</h3>
          <p className="text-slate-400 text-sm">Các lõi đa luồng đang thương thuyết</p>
        </div>
        
        <div className="relative w-64 h-64 mx-auto mb-10">
          <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100">
            <polygon points="50,15 15,85 85,85" fill="none" stroke="#334155" strokeWidth="2" strokeDasharray="4 4" className="opacity-50" />
            {activeAgent === 'CMO' && <line x1="15" y1="85" x2="85" y2="85" stroke="#ef4444" strokeWidth="3" className="animate-pulse" />}
            {activeAgent === 'CFO' && <line x1="85" y1="85" x2="50" y2="15" stroke="#f59e0b" strokeWidth="3" className="animate-pulse" />}
            {activeAgent === 'SYSTEM' && <line x1="50" y1="15" x2="15" y2="85" stroke="#10b981" strokeWidth="3" className="animate-pulse" />}
          </svg>
          
          {/* System Node */}
          <div className={`absolute top-0 left-1/2 -ml-8 w-16 h-16 rounded-full flex flex-col items-center justify-center border-2 border-slate-700 shadow-lg transition-all duration-300 ${activeAgent === 'SYSTEM' ? 'bg-indigo-600 ring-8 ring-indigo-500/30 scale-110' : 'bg-slate-800'}`}>
            <Bot className="text-white mb-1" size={18}/>
            <span className="text-[10px] font-bold text-white uppercase">SYS</span>
          </div>

          {/* CMO Node */}
          <div className={`absolute bottom-0 left-[-16px] w-16 h-16 rounded-full flex flex-col items-center justify-center border-2 border-slate-700 shadow-lg transition-all duration-300 ${activeAgent === 'CMO' ? 'bg-blue-600 ring-8 ring-blue-500/30 scale-110' : 'bg-slate-800'}`}>
            <Users className="text-white mb-1" size={18}/>
            <span className="text-[10px] font-bold text-white uppercase">CMO</span>
          </div>

          {/* CFO Node */}
          <div className={`absolute bottom-0 right-[-16px] w-16 h-16 rounded-full flex flex-col items-center justify-center border-2 border-slate-700 shadow-lg transition-all duration-300 ${activeAgent === 'CFO' ? 'bg-rose-600 ring-8 ring-rose-500/30 scale-110' : 'bg-slate-800'}`}>
        ...
        <div className="w-full max-w-sm px-4">
          <div className="flex justify-between text-xs text-slate-400 font-mono mb-2 border-b border-slate-800 pb-2">
          ...
          {progress === 100 && (
            <button 
              onClick={onComplete}
              className="mt-8 w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold text-md shadow-[0_0_20px_rgba(16,185,129,0.3)] animate-pulse flex items-center justify-center"
            >
      ...
      {/* Right: Chat Window */}
      <div className="w-full md:w-2/3 h-[600px] bg-slate-800 rounded-3xl border border-slate-700 flex flex-col overflow-hidden shadow-2xl relative">
        <div className="bg-slate-950/80 backdrop-blur-md p-5 border-b border-slate-700 flex items-center justify-between z-10">
          <h2 className="text-white font-bold flex items-center"><MessageSquare className="mr-3 text-blue-400" size={18}/> Live AI Board</h2>
          ...
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex w-full ${msg.sender === 'CMO' ? 'justify-start' : msg.sender === 'CFO' ? 'justify-end' : 'justify-center'}`}>
              <div className={`max-w-[80%] flex ${msg.sender === 'CFO' ? 'flex-row-reverse' : ''}`}>
                {msg.sender !== 'SYSTEM' && (
                  <div className={`mt-1 flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-black shadow-lg ${msg.sender==='CFO'?'ml-3 bg-rose-600 text-white':'mr-3 bg-blue-600 text-white'}`}>
                    {msg.sender}
                  </div>
                )}
                <div className={`flex flex-col ${msg.sender === 'CFO'?'items-end': msg.sender==='SYSTEM'?'items-center text-center':''}`}>
                  {msg.sender !== 'SYSTEM' && <span className="text-[10px] text-slate-400 mb-1 tracking-wide uppercase">{msg.role}</span>}
                  <div className={`p-4 text-sm leading-relaxed rounded-2xl shadow-sm border ${
                    msg.sender === 'CMO' ? 'bg-slate-700 text-slate-100 border-slate-600 rounded-tl-sm' : 
                    msg.sender === 'CFO' ? 'bg-rose-900/40 text-rose-50 border-rose-800/50 rounded-tr-sm' : 
                    'bg-slate-900 border-slate-800 text-slate-400 italic rounded-full px-6'}`}>
                    {msg.text}
                  </div>
                </div>
              </div>
